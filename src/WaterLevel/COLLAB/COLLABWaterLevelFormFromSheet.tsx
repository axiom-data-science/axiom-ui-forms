...
const CollabedWatterLevelFormSheetSchemaOnly = ({
  ...
  const schemaState = useState<JSONSchema6 | undefined>(schema)
  const fieldOverridesFromSections = formSectionOverrides
    .map((s) => getFieldsFromFormSection(s as IFormSection))
    .flat()
    .map((f) => {
      const fO = f as IFormFieldOverride
      const fieldO: Pick<IFormFieldOverride, ***REMOVED***prop***REMOVED*** | ***REMOVED***type***REMOVED*** | ***REMOVED***example***REMOVED***> = pick(fO, [
  ...
  const allCombinedFields = getFieldsFromFormSection(combinedForm).filter(
    (f) => !(f as { skip_path?: boolean }).skip_path
  )
  ...
  return (
    <>
      ...
      {(missingInCombined.length > 0 || addedInCombined.length > 0) && includeDebug && (
        <div
          className={`fixed bottom-0 left-0 ${showDebug ? ***REMOVED***w-90***REMOVED*** : ***REMOVED***w-40***REMOVED***} bg-white/90 z-80 shadow-md border-2 border-slate-200`}
        >
          <h4
            className="p-2 bg-slate-600 text-xs text-white cursor-pointer flex flex-row gap-2 items-center"
            onClick={() => setShowDebug(!showDebug)}
          >
            {showDebug ? <CaretDownIcon /> : <CaretUpIcon />} Debug info{***REMOVED*** ***REMOVED***}
            <span className="text-[10px] py-1 px-2 bg-white text-red-600 font-bold rounded-lg">
              {missingInCombined.length + addedInCombined.length}
            </span>
          </h4>
          {showDebug && (
              ...
              <div>
                ...
                <ul className="list-disc pl-5 max-h-50 overflow-auto">
                  {missingInCombined.map((fieldId) => (
                    <li key={fieldId} className="text-sm text-red-600">
                      {fieldId}
                    </li>
                  ))}
                </ul>
              ...
              <div>
                ...
                <ul className="list-disc pl-5 max-h-50 overflow-auto">
                  {addedInCombined.map((fieldId) => (
                    <li key={fieldId} className="text-sm text-green-600">
                      {fieldId}
                    </li>
                  ))}
                </ul>
...
const CollabWatterLevelFormSheetLoader = ({
  useSchemaOnly,
}: {
  useSchemaOnly?: boolean
}): ReactElement => {
  ...
  return (
    <ViewWithLoader isLoading={isLoading} error={error} data={data}>
      {data !== undefined &&
        (useSchemaOnly ? (
          <CollabedWatterLevelFormSheetSchemaOnly
            schema={data.schema}
            formSectionOverrides={data.formSectionOverrides}
          />
        ) : (
          <CollabWatterLevelFormSheet
            schema={data.schema}
            formSectionOverrides={data.formSectionOverrides}
          />
        ))}
